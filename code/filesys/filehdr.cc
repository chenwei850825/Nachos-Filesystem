...
{ 
	...
	/**@MP4 larger file**/
    if( fileSize - numBytes > 0 ){
	fileSize -= numBytes;
	sectorOfNext = freeMap->FindAndSet();
	if (sectorOfNext == -1) 		
          return FALSE;		// no free block for file header 	
	next = new FileHeader;
	if( !next->Allocate( freeMap, fileSize ))
...
FileHeader::ByteToSector(int offset)
{
	...
	offset -= MaxFileSize;
	return ( next->ByteToSector(offset) );
	}
}
